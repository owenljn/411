...
def create_new_input(input_img_matrix, datasize, n):
...
def create_training_M():
    ...
    for label in labels: # This loop separates all emotions in different training sets
        ...
        #rgb_img = np.repeat(resized_img.astype(float32), 3, 2)
        rgb_img = cv2.cvtColor(resized_img, cv2.COLOR_GRAY2RGB)
        # plt.imshow(rgb_img)
        # show()
        rgb_img = rgb_img.reshape((1, 227*227*3))
        if 'train'+str(label) in mdict.keys():
            mdict['train'+str(label)] = vstack((rgb_img, mdict['train'+str(label)]))
        else:
            mdict['train'+str(label)] = rgb_img
        i += 1 
    savemat('227training.mat', mdict)   

def create_new_inputs(i):
    output = {}
    for i in range(1, 8):
        training_matrix = create_new_input(M['train'+str(i)], M['train'+str(i)].shape[0], i)
        output['train'+str(i)] = training_matrix
    savemat('newTraining.mat', output)
...
create_training_M() # Note please run this function first, then comment this line and uncomment the lines below to avoid possible laggings.
# M = loadmat("227training.mat")
#create_new_inputs()